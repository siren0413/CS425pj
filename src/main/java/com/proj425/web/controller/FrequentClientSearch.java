...
public class FrequentClientSearch extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String start_date = args.get("start_date")[0].toLowerCase().trim();
		String end_date = args.get("end_date")[0].trim().toLowerCase();

		List<Client> client_list = null;

		try {
			client_list = booking_service.findFrequentClient(start_date, end_date);
